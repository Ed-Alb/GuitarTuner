...
DELTA_FREQ = SAMPLE_FREQ / WINDOW_SIZE # frequency step width of the interpolated DFTtext
...
def callback(indata, frames, time, status):
        ...
        if any(indata):
            ...
            # reduce white noise or noise which is very close to white noise
            # (white noise has a flat spectral distribution). This is necessar
            # y as the HPS method does not work so well if there is a lot of white noise.
            ...
            # If the spectrum is downsampled, there is a certain risk that this peak
            # is simply ignored. This can be avoided having an interpolated spectrum
            # as the peaks are "smeared" over a larger area.
            ...
            if callback.noteBuffer.count(callback.noteBuffer[0]) == len(callback.noteBuffer):
                ...
                if callback.selection_pitch - 1 <= max_freq <= callback.selection_pitch + 1:
                    result = "Tuned"
                    app.GuitarTuner.status_image_view.image = "check.png"
                elif max_freq < callback.selection_pitch:
                    result = "Tune UP"
                    app.GuitarTuner.status_image_view.image = "up.png"
                elif max_freq > callback.selection_pitch:
                    result = "Tune DOWN"
                    app.GuitarTuner.status_image_view.image = "down.png"
            else:
